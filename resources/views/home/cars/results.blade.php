            ...
            <div class="results-layout">
                    ...
                    <div class="filter-group">
                        ...
                        <div class="price-range">
                            <div class="price-inputs">
                                <input type="number" id="minPrice" name="minPrice" placeholder="من" min="0" max="1000" value="{{ old('minPrice', $searchParams['minPrice'] ?? 50) }}">
                                <span class="price-separator">-</span>
                                <input type="number" id="maxPrice" name="maxPrice" placeholder="إلى" min="0" max="1000" value="{{ old('maxPrice', $searchParams['maxPrice'] ?? 500) }}">
                            </div>
                            <div class="price-slider">
                                <input type="range" id="priceRange" min="50" max="500" value="{{ old('maxPrice', $searchParams['maxPrice'] ?? 500) }}" class="slider">
                        ...
                        <div class="filter-options">
                            @foreach([
                                ...
                                <label class="filter-option">
                                    <input type="checkbox" name="carTypes[]" value="{{ $value }}" {{ in_array($value, old('carTypes', $searchParams['carTypes'] ?? [])) ? 'checked' : '' }}>
                                    <div class="car-type-info">
                                        <span class="car-type-name">{{ $label }}</span>
                                        <span class="car-count">{{ \App\Models\Car::where('type', $value)->whereHas('rentalCompany', fn($q) => $q->where('address', 'like', '%' . $searchParams['pickupLocation'] . '%'))->count() }} سيارة</span>
                                    </div>
                                </label>
                            @endforeach
                        </div>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="filter-options">
                            @foreach($rentalCompanies as $company)
                                <label class="filter-option">
                                    <input type="checkbox" name="rentalCompanies[]" value="{{ $company->id }}" {{ in_array($company->id, old('rentalCompanies', $searchParams['rentalCompanies'] ?? [])) ? 'checked' : '' }}>
                    ...
                    <div class="filter-group">
                        ...
                        <div class="filter-options">
                            @foreach(['تكييف', 'نظام GPS', 'مقاعد جلدية', 'بلوتوث', 'شاشة لمس', 'فتحة سقف', 'كاميرا خلفية', 'نظام صوتي متقدم'] as $feature)
                                <label class="filter-option">
                                    <input type="checkbox" name="features[]" value="{{ $feature }}" {{ in_array($feature, old('features', $searchParams['features'] ?? [])) ? 'checked' : '' }}>
                                    <span>{{ $feature }}</span>
                                </label>
                            @endforeach
                        </div>
                ...
                <main class="results-main">
                    ...
                    <div class="results-toolbar">
                        ...
                        <div class="sort-options">
                            ...
                            <select id="sortBy" name="sortBy" onchange="updateSort(this.value)">
                                <option value="price-low" {{ old('sortBy', $searchParams['sortBy'] ?? 'price-low') == 'price-low' ? 'selected' : '' }}>السعر: من الأقل للأعلى</option>
                                <option value="price-high" {{ old('sortBy', $searchParams['sortBy'] ?? '') == 'price-high' ? 'selected' : '' }}>السعر: من الأعلى للأقل</option>
                    ...
                    <form id="filterForm" action="{{ route('cars.search') }}" method="POST">
                        ...
                        <input type="hidden" name="dropoffLocation" value="{{ $searchParams['dropoffLocation'] ?? '' }}">
                        ...
                        <input type="hidden" name="additionalDriver" value="{{ $searchParams['additionalDriver'] ?? '' }}">
                    ...
                    <div class="cars-list" id="cars-list">
                        @forelse($cars as $car)
                            <div class="car-card" data-price="{{ $car->price }}" data-rating="{{ $car->rating }}" data-type="{{ $car->type }}">
                                <div class="car-header">
                                    <div class="car-image">
                                        <img src="{{ asset('images/cars/' . ($car->img ?? 'default.png')) }}" alt="{{ $car->name }}">
                                    ...
                                    <div class="car-basic-info">
                                        ...
                                        <div class="car-rating">
                                            <div class="stars">
                                                @for($i = 1; $i <= 5; $i++)
                                                    <svg viewBox="0 0 24 24" fill="{{ $i <= floor($car->rating / 10) ? 'currentColor' : 'none' }}" stroke="currentColor" stroke-width="2">
                                                        <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/>
                                                    </svg>
                                                @endfor
                                            </div>
                                            <span class="rating-text">{{ number_format($car->rating / 10, 1) }} ({{ rand(50, 200) }} تقييم)</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="car-pricing">
                                    <div class="price-info">
                                        ...
                                        <div class="total-price">المجموع: {{ number_format($car->price * $rentalDays, 2) }} ريال ({{ $rentalDays }} أيام)</div>
                                        @if($cars->min('price') == $car->price)
                                            <div class="best-price">أفضل قيمة</div>
                                        @endif
                                    </div>
                                    <button class="book-btn {{ strtolower(str_replace(' ', '_', $car->rentalCompany->name)) }}" onclick="selectCar('{{ $car->id }}', '{{ $car->name }}', '{{ $car->price }}', '{{ $car->price * $rentalDays }}')">
                                        <span>احجز الآن</span>
                                        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                            <path d="M5 12h14M12 5l7 7-7 7"/>
                                        </svg>
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form class="payment-form" action="{{ route('cars.book') }}" method="POST">
                    ...
                    <input type="hidden" name="return_location" value="{{ $searchParams['rentalType'] == 'different-location' && isset($searchParams['dropoffLocation']) ? $searchParams['dropoffLocation'] : $searchParams['pickupLocation'] }}">
                    ...
                    <input type="hidden" name="extras" id="extras-input" value="{{ json_encode(array_filter([
