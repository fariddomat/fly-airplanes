<x-app-layout>
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">
            @lang('site.create') @lang('site.dashboard.hotels')
        </h1>
        ...
        <form action="{{ route('dashboard.hotels.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
                ...
                <input type="number" name="star_rating" value="{{ old('star_rating') }}" class="w-full border border-gray-300 rounded p-2" >
                @error('star_rating')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="number" name="price_per_night" value="{{ old('price_per_night') }}" class="w-full border border-gray-300 rounded p-2" >
                ...
                <textarea name="amenities" class="w-full border border-gray-300 rounded p-2" placeholder="Enter JSON data">{{ old('amenities') }}</textarea>
                @error('amenities')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
