            ...
            @csrf
                ...
                <select name="car_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($cars as $option)
                        <option value="{{ $option->id }}" {{ old('car_id') == $option->id ? 'selected' : '' }}>{{ $option->car_model }}</option>
                    @endforeach
                </select>
                @error('car_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <input type="datetime-local" name="pickup_date" value="{{ old('pickup_date') }}" class="w-full border border-gray-300 rounded p-2">
                @error('pickup_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.return_date')</label>
                <input type="datetime-local" name="return_date" value="{{ old('return_date') }}" class="w-full border border-gray-300 rounded p-2">
                @error('return_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
