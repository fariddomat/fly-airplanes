            ...
            @method('PUT')
                ...
                <select name="flight_id" class="w-full border border-gray-300 rounded p-2">
                    ...
                    @foreach ($flights as $option)
                        <option value="{{ $option->id }}" {{ $booking->flight_id == $option->id ? 'selected' : '' }}>{{ $option->departure_airport_id  }} - {{ $option->arrival_airport_id }}</option>
                    @endforeach
                </select>
                @error('flight_id')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                ...
                <input type="datetime-local" name="booking_date" value="{{ old('booking_date', $booking->booking_date ? $booking->booking_date->format('Y-m-d\TH:i') : '') }}" class="w-full border border-gray-300 rounded p-2">
                @error('booking_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
