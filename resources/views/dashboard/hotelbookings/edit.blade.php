<x-app-layout>
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">
            @lang('site.edit') @lang('site.dashboard.hotelbookings')
        </h1>

        <form action="{{ route('dashboard.hotelbookings.update', $hotelbooking->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                        <div class="mb-4">
                ...
                <input type="date" name="check_in_date" value="{{ old('check_in_date', $hotelbooking->check_in_date) }}" class="w-full border border-gray-300 rounded p-2">
                @error('check_in_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.check_out_date')</label>
                <input type="date" name="check_out_date" value="{{ old('check_out_date', $hotelbooking->check_out_date) }}" class="w-full border border-gray-300 rounded p-2">
                @error('check_out_date')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.room_type')</label>
                <input type="text" name="room_type" value="{{ old('room_type', $hotelbooking->room_type) }}" class="w-full border border-gray-300 rounded p-2">
                @error('room_type')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.total_price')</label>
                <input type="number" name="total_price" value="{{ old('total_price', $hotelbooking->total_price) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('total_price')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
