<x-app-layout>
    <div class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-4">
            @lang('site.create') @lang('site.dashboard.airports')
        </h1>

        <form action="{{ route('dashboard.airports.store') }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            @csrf
                        <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.airport_code')</label>
                <input type="text" name="airport_code" value="{{ old('airport_code') }}" class="w-full border border-gray-300 rounded p-2">
                @error('airport_code')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.airport_name')</label>
                <input type="text" name="airport_name" value="{{ old('airport_name') }}" class="w-full border border-gray-300 rounded p-2">
                @error('airport_name')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                ...
                <input type="text" name="latitude" value="{{ old('latitude') }}" class="w-full border border-gray-300 rounded p-2">
                @error('latitude')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.longitude')</label>
                <input type="text" name="longitude" value="{{ old('longitude') }}" class="w-full border border-gray-300 rounded p-2">
                @error('longitude')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
