    ...
    <div class="container mx-auto p-6">
        ...
        <form action="{{ route('dashboard.cars.update', $car->id) }}" method="POST" class="bg-white p-6 rounded-lg shadow-md" enctype="multipart/form-data">
            ...
            @method('PUT')
                ...
                <input type="text" name="car_model" value="{{ old('car_model', $car->car_model) }}" class="w-full border border-gray-300 rounded p-2">
                @error('car_model')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.car_make')</label>
                <input type="text" name="car_make" value="{{ old('car_make', $car->car_make) }}" class="w-full border border-gray-300 rounded p-2">
                @error('car_make')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>            <div class="mb-4">
                <label class="block text-sm font-medium text-gray-700">@lang('site.car_year')</label>
                <input type="number" name="car_year" value="{{ old('car_year', $car->car_year) }}" class="w-full border border-gray-300 rounded p-2" >
                @error('car_year')
                    <span class="text-red-500 text-sm">{{ $message }}</span>
                @enderror
            </div>
